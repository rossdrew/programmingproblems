...
private fun solutionA(input: String): Any {
    return input.split('\n').map { entry ->
        ...
        val rangeDefinition = criteria[0].split('-')
        val range = (rangeDefinition[0].toInt()..rangeDefinition[1].toInt())
        ...
        }.count() in passwordDefinition.range
    }.count()
}
...
private fun solutionB(input: String): Any {
    return input.split('\n').map { entry ->
        ...
        val character = criteria[1].trim()[0]
        val password = definition[1].trim()
        ...
        passwordDefinition.validPositions.filter {
            i -> passwordDefinition.password[i-1] == passwordDefinition.character
        }.count() == 1
    }.count()
}
