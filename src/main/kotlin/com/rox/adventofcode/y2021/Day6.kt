...
fun main() {
    ...
    println("Part A: ${solutionA(puzzleInputFromFile("src/main/kotlin/com/rox/adventofcode/y2021/day6.input"))}")
    println("Part B: ${solutionB(puzzleInputFromFile("src/main/kotlin/com/rox/adventofcode/y2021/day6.input"))}")
}
...
private fun solutionA(input: String): Any {
    ...
    val allTheFish = rows[0].split(",").map { fishDefinition ->
        Lanternfish(overriddenCountdown = fishDefinition.toInt())
    }.toMutableList()

    for (day in (1 .. 80)){
        val nextGeneration = mutableListOf<Lanternfish>()
        for (fish in allTheFish){
            val fishSpawn = fish.moveToNextDay()
            if (fishSpawn.isPresent)
                nextGeneration.add(fishSpawn.get())
        }
        allTheFish.addAll(nextGeneration)
    }

    return allTheFish.size
}

class Lanternfish{
    private var spawnCountdown = 8;

    constructor(overriddenCountdown: Int = 8){
        spawnCountdown = overriddenCountdown
    }

    fun moveToNextDay(): Optional<Lanternfish>{
        spawnCountdown--
        if (spawnCountdown == -1){
            spawnCountdown = 6
            return Optional.of(Lanternfish())
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (spawnCountdown != other.spawnCountdown) return false
...
private fun solutionB(input: String): Any {
    ...
    for (day in (1 .. 256)){
        //New parents
        val nextGenerationOfFish = LongArray(size = 9)
        nextGenerationOfFish[8] = allTheFishAsNothingButANumber[0] //Births
        nextGenerationOfFish[6] = allTheFishAsNothingButANumber[0] //Resets

        //Every other fish
        for (i in 1 until allTheFishAsNothingButANumber.size){
            nextGenerationOfFish[i-1] = nextGenerationOfFish[i-1] + allTheFishAsNothingButANumber[i]
        }
        allTheFishAsNothingButANumber = nextGenerationOfFish
    }

    return allTheFishAsNothingButANumber.sum()
}
