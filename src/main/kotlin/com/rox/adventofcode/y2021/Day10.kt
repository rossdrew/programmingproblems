...
private fun solutionA(input: String): Any {
    ...
    row@for (row in rows) {
        ...
        col@for (c in row){
            if ("([{<".contains(c)){ //Open chunk
                stack.push(c)
            }else if (")]}>".contains(c)){ //Close chunk
                if (stack.peek() == inverseOf(c)){
                    stack.pop()
                }else{
                    /*DEBUG*///println("Expected '${stack.peek()}', got '${c}' for \"${row}\" +${scoreOf(c)}")
                    sum += scoreOf(c)
                    continue@row
                }
            }
...
fun valueOf(c: Char): Int {
    return when(c){
        ...
        '}' -> 3
        '>' -> 4
        else -> throw RuntimeException("Unknown block type '$c'")
    }
}
...
fun scoreOf(c: Char): Int {
    return when(c){
        ...
        '}' -> 1197
        '>' -> 25137
        else -> throw RuntimeException("Unknown block type")
    }
}

fun inverseOf(c: Char): Char {
    return when(c){
        '(' -> ')'
        ')' -> '('
        '[' -> ']'
...
private fun solutionB(input: String): Any {
    ...
    var scores = mutableListOf<BigInteger>()
    ...
    row@for (row in rows) {
        ...
        val lineScore = stack
            .map { b-> valueOf(inverseOf(b)).toBigInteger() }
            .fold(BigInteger.ZERO) { acc, v -> (acc.multiply(BigInteger("5"))).add(v) }
