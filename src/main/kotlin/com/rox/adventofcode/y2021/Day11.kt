...
fun main() {
    ...
    println("Part A: ${solutionB(inputA)}")
}
...
private fun solutionA(input: String): Any {
    ...
    var ecosystem = parseGrid(rows)
    var sum = 0

    for (iteration in 1 .. 100){
        val progressedEcosystem = progressEcosystem(ecosystem)
        ecosystem = progressedEcosystem.first
        sum += progressedEcosystem.second
    }

    return sum
}

fun progressEcosystem(ecosystem: Array<IntArray>): Pair<Array<IntArray>, Int>{
    var progressedEcosystem = ecosystem.clone()

    var sum = 0

    rows@ for (y in ecosystem.indices) {
        cols@ for (x in ecosystem[0].indices) {
            progressCell(y, x, progressedEcosystem)
        }
    }

    rows@ for (y in ecosystem.indices) {
        cols@ for (x in ecosystem[0].indices) {
            sum += flashCell(y, x, progressedEcosystem)
        }
    }

    return Pair(progressedEcosystem, sum)
}

fun flashCell(y: Int, x: Int, ecosystem: Array<IntArray>): Int {
    if (ecosystem[y][x] > 9){
        ecosystem[y][x] = 0
        return 1
    }
...
fun progressCell(y: Int, x: Int, ecosystem: Array<IntArray>){
    if (y >= ecosystem.size || y < 0) return
    if (x >= ecosystem[0].size || x < 0) return

    if (++ecosystem[y][x] == 10){
        gridNeighbours(y,x).forEach{ c-> progressCell(c.first, c.second, ecosystem) }
    }
}

fun gridNeighbours(y: Int, x: Int): List<Pair<Int, Int>> {
    return cartesianProduct(listOf(y-1,y,y+1), listOf(x-1,x,x+1)).filter { c -> (c.first != y || c.second != x) }
}
...
private fun solutionB(input: String): Any {
    ...
    for (iteration in 1 .. Int.MAX_VALUE){
        val progressedEcosystem = progressEcosystem(ecosystem)
        ecosystem = progressedEcosystem.first
        if (progressedEcosystem.second == (ecosystem.size * ecosystem[0].size))
            return iteration
        sum += progressedEcosystem.second
    }

    throw RuntimeException("Shouldn't have gotten here")
}
