...
fun main() {
    ...
    println("Part A: ${solutionA(puzzleInputFromFile("src/main/kotlin/com/rox/adventofcode/y2021/day3.input"))}")
    println("Part B: ${solutionB(puzzleInputFromFile("src/main/kotlin/com/rox/adventofcode/y2021/day3.input"))}")
}
...
private fun solutionA(input: String): Any {
    ...
    val commonalityScore = calculateCommonality(rows)

    val gammaChars = commonalityScore.map { b -> if (b > 0) '1' else '0' }.joinToString(separator = "")
    val epsilonChars = commonalityScore.map { b -> if (b < 0) '1' else '0' }.joinToString(separator = "")

    val gamma = Integer.parseInt(gammaChars, 2)
    val epsilon = Integer.parseInt(epsilonChars,2)

    return gamma * epsilon
}

fun calculateCommonality (rows : List<String>) : IntArray {
    val data = rows[0].length
    val commonalityScore = IntArray(data)

    for (row in rows){
        for (i in 0 until data){
            val bitValue = "${row[i]}".toInt()
            if (bitValue==0){
                commonalityScore[i]--
            }else if (bitValue==1){
                commonalityScore[i]++
...
private fun solutionB(input: String): Any {
    ...
    var index = 0
    var list = rows
    while (list.size != 1 && index < rows[0].length){
        list = list.filter { row -> row[index] == mostCommonDigits(list)[index] }.toList()
        index++
    }
    val oxygenGeneratorRating = Integer.parseInt(list[0], 2)

    index = 0
    list = rows
    while (list.size != 1 && index < rows[0].length){
        list = list.filter { row -> row[index] == leastCommonDigits(list)[index] }.toList()
        index++
    }

    val co2ScrubberRating = Integer.parseInt(list[0], 2)

    return oxygenGeneratorRating * co2ScrubberRating
}

fun mostCommonDigits(rows: List<String>): List<Char> {
    val commonalityScore = calculateCommonality(rows)
    return commonalityScore.map { b -> if (b >= 0) '1' else '0' }
}

fun leastCommonDigits(rows: List<String>): List<Char> {
    val commonalityScore = calculateCommonality(rows)
    return commonalityScore.map { b -> if (b < 0) '1' else '0' }
}
