...
val priorityRange = alphabet + alphabet.toUpperCase()

fun main() {
    for (i in 1 .. 52){
        priorities[priorityRange[i-1]] = i
    }

    println("Sample Input A: ${solutionA(inputSample)}")
    println("Sample Input B: ${solutionB(inputSample)}")
    println("Part A: ${solutionA(puzzleInputFromFile("src/main/kotlin/com/rox/adventofcode/y2022/Day3.input"))}")
    println("Part B: ${solutionB(puzzleInputFromFile("src/main/kotlin/com/rox/adventofcode/y2022/Day3.input"))}")
}
...
private fun solutionA(input: String): Any {
    ...
    val values = rows.map { row ->
        val halfRow = row.length / 2
        val leftHalf = row.subSequence(0, halfRow).toSet()
        val rightHalf = row.subSequence(halfRow until row.length).toSet()
        val intersection = leftHalf.intersect(rightHalf)
        priorities[intersection.first()] as Int
    }
    return values.sum()
}
