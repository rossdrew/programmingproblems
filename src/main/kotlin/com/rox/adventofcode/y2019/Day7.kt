...
private fun solutionA(input : String) : Any {
    ...
    val phaseConfigurationResults: MutableMap<PhaseConfiguration, Int> = mutableMapOf()

    //Rough, unoptimized code for moving to when I have problem 5 done
    for (phaseA in 0..4) {
        val resultA = runProgram(program.toMutableList(), phaseA, 0)
        for (phaseB in 0..4){
            val resultB = runProgram(program.toMutableList(), phaseB, resultA)
            for (phaseC in 0..4){
                val resultC = runProgram(program.toMutableList(), phaseC, resultB)
                for (phaseD in 0..4){
                    val resultD = runProgram(program.toMutableList(), phaseD, resultC)
                    for (phaseE in 0..4) {
                        val resultD = runProgram(program.toMutableList(), phaseE, resultD)
                        phaseConfigurationResults[PhaseConfiguration(phaseA,phaseB,phaseC,phaseD,phaseE)] = resultD
                    }
    ...
    println(phaseConfigurationResults.maxBy { it.value })

    throw RuntimeException("Unimplmeneted")
}

private data class PhaseConfiguration(val a: Int, val b: Int, val c: Int, val d: Int, val e:Int)
